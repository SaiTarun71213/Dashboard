    ...
    <div
      ...
      <div
        *ngIf="showGrid"
        ...
        [style.background-image]="gridBackgroundStyle">
      </div>
...
export class CustomGridComponent implements OnInit, OnDestroy {
  ...
  onItemResizeMove(event: any): void {
    // Update item size and position in real-time
    if (event.size || event.position) {
      const updates: Partial<GridItem> = {};

      if (event.size) {
        updates.width = event.size.width;
        updates.height = event.size.height;
      }

      if (event.position) {
        updates.x = event.position.x;
        updates.y = event.position.y;
      }

      this.gridEngine.updateItem(event.item.id, updates);
    }
  }
  ...
  private updateGridBackground(): void {
    ...
    this.containerBackgroundColor = this.config.gridBackgroundColor || 'transparent';
    ...
    // Create custom grid lines based on config
    const lineColor = this.config.gridLineColor || '#e0e0e0';
    const lineWidth = this.config.gridLineWidth || 1;
    const lineStyle = this.config.gridLineStyle || 'solid';
